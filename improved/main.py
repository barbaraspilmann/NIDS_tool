...
def run_hybrid_pipeline(
    ...
    if "Rate" not in df.columns:
        ...
        if not df_flagged_s1.empty:
            ...
            # Some CICIoT2023 flows have a near-zero inter-arrival time
            # (packets sent back-to-back), which makes the dataset's own
            # Rate feature come out as literal infinity. Left unfiltered
            # this printed "Rate range: 1000.0 - inf pps" in the report,
            # which looks like a bug rather than a real burst. Report the
            # finite range and call out the unbounded ones separately.
            finite_rate = rate_col[np.isfinite(rate_col)]
            n_unbounded = int((~np.isfinite(rate_col)).sum())

            if not finite_rate.empty:
                range_str = f"{finite_rate.min():,.1f} – {finite_rate.max():,.1f} pps  (mean: {finite_rate.mean():,.1f} pps)"
            else:
                range_str = "unbounded for every flagged flow"

            unbounded_note = (
                f"  {n_unbounded:,} of these had an unbounded rate "
                f"(near-instant packet bursts)."
                if n_unbounded else ""
            )
            ...
            all_incidents.append({
                ...
                "details": (
                    ...
                    f"{config.ML_RATE_THRESHOLD:,.1f} pps.  "
                    f"Rate range: {range_str}.{unbounded_note}  "
