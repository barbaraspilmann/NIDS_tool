...
# The CICIoT2023 CSV stores labels in all caps (e.g. "DDOS-PSHACK_FLOOD"),
# but MITRE_MAPPING above uses mixed case for readability. A plain dict
# lookup was silently missing every single row and always falling back to
# _MITRE_UNKNOWN. Building an upper-cased lookup once at import time fixes
# this without changing how MITRE_MAPPING itself is written or read.
_MITRE_MAPPING_UPPER = {key.upper(): value for key, value in MITRE_MAPPING.items()}


def _lookup_mitre(attack_label: str) -> Dict[str, str]:
    """Looks up the MITRE ATT&CK entry for a label, ignoring case."""
    return _MITRE_MAPPING_UPPER.get(attack_label.upper(), _MITRE_UNKNOWN)
...
class MLDetectionEngine:
    ...
    def predict_anomalies(
        ...
        for row_idx in top_indices:
            ...
            # Note: deliberately NOT falling back to the "index" column here.
            # After reset_index(drop=False), df_reset always has a column
            # called "index" holding the row's raw position (e.g. 712220) —
            # if that gets used as a timestamp fallback it just prints a
            # meaningless number instead of a real time. CICIoT2023's
            # flow-feature CSVs don't carry a timestamp at all, so "N/A" is
            # the honest answer.
            raw_ts    = self._get_field(row, ["Timestamp", "timestamp"], "N/A")
            ...
            # _lookup_mitre() matches case-insensitively since the dataset's
            # own labels are all uppercase (see _MITRE_MAPPING_UPPER above).
            raw_label = self._get_field(row, ["Label", "label", "Category"], None)
            attack_label = str(raw_label) if raw_label is not None else "Unknown"
            mitre_entry = _lookup_mitre(attack_label)
